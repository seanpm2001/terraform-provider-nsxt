...
func NewUserInfoClient(connector vapiProtocolClient_.Connector) *userInfoClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.aaa.user_info")
...
func (uIface *userInfoClient) Get(orgIdParam string, projectIdParam string) (nsx_policyModel.UserInfo, error) {
	...
	methodResult := uIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.aaa.user_info", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.UserInfo
