...
type StatisticsClient interface {
	...
	Get(orgIdParam string, projectIdParam string, vpcIdParam string, subnetIdParam string) (nsx_policyModel.SubnetStatistics, error)
}
...
func NewStatisticsClient(connector vapiProtocolClient_.Connector) *statisticsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.vpcs.subnets.statistics")
...
func (sIface *statisticsClient) Get(orgIdParam string, projectIdParam string, vpcIdParam string, subnetIdParam string) (nsx_policyModel.SubnetStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.SubnetStatistics
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpcs.subnets.statistics", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.SubnetStatistics
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.SubnetStatistics), nil
	} else {
